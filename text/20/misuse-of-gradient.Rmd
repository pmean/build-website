---
title: "Blog post: An example of a poor color choice"
author: "Steve Simon"
source: "New"
date: "2020-03-11"
...
![](http://www.pmean.com/images/20/misuse-of-gradient.png)
...
```{r color-circle1, echo=FALSE, fig.width=5, fig.height=5}
...
delta <- 0.9
angle_seq <- (1:n)/n
df0<- data.frame(x=delta*sin(2*pi*angle_seq), y=delta*cos(2*pi*angle_seq))
sb <- (n/k)*(1:k)
df1 <- df0[sb, ]

ggplot(df0, aes(x, y)) +
  geom_point(color=hsv(angle_seq, 1, 1), size=2, shape=16) +
  geom_point(data=df1, aes(x, y), color=hsv(sb/n, 1, 1), size=24, shape=16) +
...
```{r color-circle2, echo=FALSE, fig.width=5, fig.height=5}
ggplot(df0, aes(x, y)) +
  geom_point(color=hsv(angle_seq, 1, 0.5), size=2, shape=16) +
  geom_point(data=df1, aes(x, y), color=hsv(sb/n, 1, 0.5), size=24, shape=16) +
  expand_limits(x=c(-1, 1), y=c(-1, 1)) +
  theme_void() +
  theme(
    panel.background=element_rect(fill="#808080"))
...
```{r color-circle3, echo=FALSE, fig.width=5, fig.height=5}
ggplot(df0, aes(x, y)) +
  geom_point(color=hsv(angle_seq, 0.5, 1), size=2, shape=16) +
  geom_point(data=df1, aes(x, y), color=hsv(sb/n, 0.5, 1), size=24, shape=16) +
  expand_limits(x=c(-1, 1), y=c(-1, 1)) +
  theme_void() +
  theme(
    panel.background=element_rect(fill="#808080"))
