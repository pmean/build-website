...
You need to make a projection any time you try to represent a three dimensional object like the state of Colorado on a two dimensional medium like a computer screen or a sheet of paper. The most common projection is the Mercator projection. Here is the Mercator projection for the state of Colorado.

```{r mercator}
projection_string <- "+proj=merc +lon_0=0 +k=1 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs"
projection_string <- "+proj=merc +lon_0=0 +k=1 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +no_defs"
co %>%
  ggplot() + 
...
```{r laea}
projection_string <- "+proj=merc +lon_0=0 +k=1 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs"
projection_string <- "+proj=laea +lon_0=-105.5 +lat_0=39 +ellps=WGS84 +datum=WGS84 +no_defs"
co %>%
  ggplot() + 
  geom_sf(color="red", fill="white") +
  coord_sf(crs=projection_string)
