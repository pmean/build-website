...
```{r}
display_terms <- function(df, x, y, lb, z=y) {
  full_data <- data.frame(x=df[ , x], y=df[ , y], z=df[ , z])
  integer_data <- full_data[1:21, ]
  ggplot(full_data, aes(x, y)) +
      ggtitle(lb) +
      geom_line() +
      geom_point(aes(x, z), na.rm=TRUE) +
      xlab(" ") +
      ylab(" ")
}
...
lb <- paste(lb1, lb2)
for (j in 1:16) {
  xm %>% display_terms("c2", paste0("c", j), lb[j]) -> cplot
  plot(cplot)
}
```
...
lm(y~c2+c3+c4, data=xm) %>%
  augment(newdata=xm) %>%
  data.frame %>%
  display_terms("c2", ".fitted", "Single cubic fit", "y")
```
