...
You can get a variety of splines by defining constant, linear, quadratic, and cubic terms and then shift those functions to the right with zeros to the left.
...
for (j in 1:16) {
  xm[1:21, ] %>%
    ggplot(aes(c2, .data[[paste0("c", j)]])) +
      ggtitle(paste(lb1[j], lb2[j])) +
      geom_point() +
      xlab(" ") +
      ylab(" ") -> cplot
  plot(cplot)
