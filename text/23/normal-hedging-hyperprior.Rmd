...
The parameter $\tau$ is called the precision. The normal density function in terms of $\tau$ is

$f(X) = \frac{\tau}{\sqrt{2 \pi}}e^{- \tau^2 (X-\mu)^2}$

Simplify this by removing any constants not involving $X$, $\mu$, or $\tau$.

$f(X) \propto \tau e^{- \tau^2 (X-\mu)^2}$

You have an informative prior on $\mu$ that also has a normal distribution

$g(\mu; \mu_0, \tau_0) \propto \tau_0 e^{-\tau_0^2 (\mu-\mu_0)^2}$

You have collected a sample of n data points, $\mathbf{X} = X_1, X_2, ..., X_n$. This gives you a likelihood of 

$L(\mathbf{X} | \mu) \propto \tau^n e^{-\tau^2\sum(X_i-\mu)^2}$

Rewrite this by incorporating $\bar{X}$ into the squared term

$L(\mathbf{X} | \mu) \propto \tau^n e^{-\sum \tau (X_i-\bar{X}+\bar{X}-\mu)^2}$

When you square the term inside the sum, the cross-product cancels out, leaving you with

$L(\mathbf{X} | \mu) \propto \tau^n e^{-\tau^2(\sum(X_i-\bar{X})^2} e^{-\tau^2 n (\bar{X}-\mu)^2}$

$L(\mathbf{X} | \mu) \propto \tau^n e^{-\tau^2((n-1)S^2} e^{-\tau^2 n (\bar{X}-\mu)^2}$

The posterior distribution is proportional to the product of the prior and the likelihood.

$h(\mu | \mathbf{X}) \propto \tau_0 e^{-\tau_0^2(\mu-\mu_0)^2} \tau^n e^{-\tau((n-1)S^2} e^{-\tau^2\sum(\bar{X}-\mu)^2)}$
...
$\frac{\tau n\bar{X}\beta + \tau_0 \mu_0}{n\tau+\tau0}$
