...
Assume for simplicity that $\sigma$ is known and equal to $\sigma_F$. A more general case, with $\sigma$ unknown is described on a separate web page.

The mathematical details are a bit easier to follow if you let

$\tau_F = \frac{1}{\sigma_F^2}$

The parameter $\tau$ is called the precision. Note that some define the precision differently as the inverse of the standard deviation rather than the inverse of the variance, The normal density function in terms of $\tau_F$ is

$f(X; \mu, \tau_F) = \sqrt{\frac{\tau_F}{2 \pi}}e^{- \frac{1}{2}\tau_F (X-\mu)^2}$

Simplify this by removing any constants not involving $X$, $\mu$, or $\tau_F$.

$f(X) \propto e^{- \frac{1}{2} \tau_F (X-\mu)^2}$

You have an informative prior on $\mu$ that also has a normal distribution with a prior mean $\mu_0$ and a prior precision $\tau_0$.
...
[Note that the following formulas need a lot of work.]

Drop the middle exponential term, which is constant with respect to $\mu$. Set

$\mu_W=\frac{\tau_0 \mu_0 + n \tau_F \bar{X}}{\tau_0 + n \tau_F}$

and rewrite the other two exponentials as 

$h(\mu | \mathbf{X}) \propto \tau_0 e^{-\tau_0^2(\mu - \mu_W + \mu_W - mu_0)^2} \tau_F^n e^{-\tau_F(n-1)S^2} e^{-n \tau_F^2(\mu - \mu_W + \mu_W - \bar{X})^2)}$

which simplifies to

$h(\mu | \mathbf{X}) \propto e^{-(n\tau_F+\tau0)^2(\mu-\frac{n \tau_F \bar{X} + \tau_0 \mu_0}{n\tau_F+\tau0})^2}$

which, thanks to the miracle of conjugancy, is a normal distribution. The posterior mean

$\frac{n \tau_F \bar{X} + \tau_0 \mu_0}{n\tau_F+\tau0}$
...
prior05 <- qnorm(0.025, mu0, tau0) 
prior95 <- qnorm(0.975, mu0, tau0) 

post05 <- qnorm(0.025, mu1, tau1) 
post95 <- qnorm(0.025, mu1, tau1) 
...
The posterior distribution is not close to either the prior distribution or the likelihood. It places $\theta$ within `r sprintf("%.2f", post05)` and `r sprintf("%.2f", post95)` with 95% probability. It excludes the prior mean of `r sprintf("%.2f", mu0)` and it excludes `r xbar`, the mean of the data.
