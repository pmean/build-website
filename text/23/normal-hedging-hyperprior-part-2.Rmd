...
$f(X; \mu, \sigma) = \frac{\tau}{\sigma sqrt{2 \pi}}e^{-(X-\mu)^2 / 2\sigma^2}$
...
Assume that $\mu$ is known and equal to $\mu_F$. An earlier case with $\tau$ known and $\mu$ unknown appears on a different web page. A more general case, with both $\mu$ and $\tau$ unknown is also described on a separate web page. For simplicity, we will drop any multiplicative constants not involving X or $\tau$

$f(X; \tau) \propto \sqrt{\tau}e^{-\frac{1}{2}\tau(X-\mu_F)^2}$

The conjugate prior for a normal distribution is a gamma distribution.

$g(\tau; \alpha_0, \beta_0) \propto \frac{\beta_0^\alpha_0}{\Gamma(\alpha)} \tau^{\alpha-1} e^{-\beta \tau}$

where 

$\beta_0 = 1 / \tau_0$

For an informative prior distribution, $\alpha$ is large. This prior is is equivalent to adding $n_0$ pseudo data values of $\tau_0$ to the observed data.
...
$h(\mu | \mathbf{X}) \propto e^{-(n\tau_F+\tau0)(\mu-\frac{n \tau_F \bar{X} + \tau_0 \mu_0}{n\tau_F+\tau0})^2}$
...
The posterior distribution is not close to either the prior distribution or the likelihood. It places $\theta$ within `r sprintf("%.2f", post05)` and `r sprintf("%.2f", post95)` with 95% probability. It excludes the prior mean of `r mu0` and it excludes `r xbar`, the mean of the data.
