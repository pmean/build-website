...
graph_without_axis <- function(x, lab, limits=range(x)) {
  ...
  plot((1:2048)/2048, x, 
       xlim=c(0, 1.2), 
       ylim=c(limits[1]*1.1, limits[2]), # adjust lower limit
       type="l",
       axes=FALSE)
  text(1.1, x[2048], lab, adj=0.5)
}
...
wt.comp <- function(x, y) {
  ...
  as.vector(midwt(single_wavelet, d4.rwt$h.0, L=8)$x)
}
```

```{r calculate-functions}
calculate_mother <- function(i, L) {
  yl <- rep(0, 2048)
  yh <- matrix(0, nrow=2048, ncol=L)
  yh[i, L] <- 1
  as.vector(mirdwt(yl, yh, d4.rwt$h.0, L)$x)
}
...
```{r wrapped}
{
  graph_without_axis(calculate_father(2^10+2^9+1+2^8+1, 7),"d4 father")
  graph_without_axis(calculate_mother(2^10+2^9+1+2^8+1, 7),"d4 mother")
}
...
Note: To get the math to work on my end, tThe low pass filter provided by the rwt package needs to be negated. 
...
First you dilute the W and V values at the top of the pyramid. Then you compute a running dot product of the reversed g vector with the diluted V9 values plus a running dot product of the reversed h vector with the diluted W9 values.
...
V8_signal <- as.vector(midwt(V8_coefficients, d4.rwt$h.0, L=8)$x)

V7_coefficients <- rep(0, 2048)
V7_coefficients[1:16] <- V7
V7_signal <- as.vector(midwt(V7_coefficients, d4.rwt$h.0, L=7)$x)

V6_coefficients <- rep(0, 2048)
V6_coefficients[1:32] <- V6
V6_signal <- as.vector(midwt(V6_coefficients, d4.rwt$h.0, L=6)$x)

wt.5 <- mdwt(ecg, g, L=5)
V5 <- wt.5$y[1:64] 
V5_coefficients <- rep(0, 2048)
V5_coefficients[1:64] <- V5
V5_signal <- as.vector(midwt(V5_coefficients, d4.rwt$h.0, L=5)$x)

wt.4 <- mdwt(ecg, g, L=4)
V4 <- wt.4$y[1:128] 
V4_coefficients <- rep(0, 2048)
V4_coefficients[1:128] <- V4
V4_signal <- as.vector(midwt(V4_coefficients, d4.rwt$h.0, L=4)$x)

wt.3 <- mdwt(ecg, g, L=3)
V3 <- wt.3$y[1:256] 
V3_coefficients <- rep(0, 2048)
V3_coefficients[1:256] <- V3
V3_signal <- as.vector(midwt(V3_coefficients, d4.rwt$h.0, L=3)$x)

wt.2 <- mdwt(ecg, g, L=2)
V2 <- wt.2$y[1:512] 
V2_coefficients <- rep(0, 2048)
V2_coefficients[1:512] <- V2
V2_signal <- as.vector(midwt(V2_coefficients, d4.rwt$h.0, L=2)$x)

wt.1 <- mdwt(ecg, g, L=1)
V1 <- wt.1$y[1:1024] 
V1_coefficients <- rep(0, 2048)
V1_coefficients[1:1024] <- V1
V1_signal <- as.vector(midwt(V1_coefficients, d4.rwt$h.0, L=1)$x)
...
W9_signal <- as.vector(midwt(W9_coefficients, d4.rwt$h.0, L=9)$x)

W8_coefficients <- rep(0, 2048)
W8_coefficients[9:16] <- W8
W8_signal <- as.vector(midwt(W8_coefficients, d4.rwt$h.0, L=9)$x)

W7_coefficients <- rep(0, 2048)
W7_coefficients[17:32] <- W7
W7_signal <- as.vector(midwt(W7_coefficients, d4.rwt$h.0, L=9)$x)

W6_coefficients <- rep(0, 2048)
W6_coefficients[33:64] <- W6
W6_signal <- as.vector(midwt(W6_coefficients, d4.rwt$h.0, L=9)$x)

W5_coefficients <- rep(0, 2048)
W5_coefficients[65:128] <- W5
W5_signal <- as.vector(midwt(W5_coefficients, d4.rwt$h.0, L=9)$x)

W4_coefficients <- rep(0, 2048)
W4_coefficients[129:256] <- W4
W4_signal <- as.vector(midwt(W4_coefficients, d4.rwt$h.0, L=9)$x)

W3_coefficients <- rep(0, 2048)
W3_coefficients[257:512] <- W3
W3_signal <- as.vector(midwt(W3_coefficients, d4.rwt$h.0, L=9)$x)

W2_coefficients <- rep(0, 2048)
W2_coefficients[513:1024] <- W2
W2_signal <- as.vector(midwt(W2_coefficients, d4.rwt$h.0, L=9)$x)

W1_coefficients <- rep(0, 2048)
W1_coefficients[1025:2048] <- W1
W1_signal <- as.vector(midwt(W1_coefficients, d4.rwt$h.0, L=9)$x)
