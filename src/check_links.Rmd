...
full_file_name <- paste0(new_path, md_files$yr, "/", md_files$fn)
md_links <- NULL
for (i_fn in full_file_name) {
  ...
  md_file <- str_replace(i_fn, new_path, "")
...
full_file_name <- paste0(old_path, html_files$yr, "/", html_files$fn)
html_links <- NULL
for (i_fn in full_file_name) {
  read_lines(i_fn) %>%
  ...
  html_file <- str_replace(i_fn, old_path, "")
